                   ...
                   size_t mr_size, int port)
{
    ...
    if (event->event != RDMA_CM_EVENT_CONNECT_REQUEST) {
        LOG_ERR("expected CONNECT_REQUEST, got %s", rdma_event_str(event->event));
    ...
    if (event->event != RDMA_CM_EVENT_ESTABLISHED) {
        LOG_ERR("expected ESTABLISHED, got %s", rdma_event_str(event->event));
        rdma_ack_cm_event(event); goto out;
    }
    rdma_ack_cm_event(event);
                   ...
                   size_t mr_size, const char *server_ip, int port)
{
    ...
    if (rdma_get_cm_event(ec, &event)) { LOG_ERR("rdma_get_cm_event failed"); goto out; }
    if (event->event != RDMA_CM_EVENT_ADDR_RESOLVED) {
        LOG_ERR("expected ADDR_RESOLVED, got %s", rdma_event_str(event->event));
        rdma_ack_cm_event(event); goto out;
    }
    rdma_ack_cm_event(event);
    ...
    if (event->event != RDMA_CM_EVENT_ROUTE_RESOLVED) {
        LOG_ERR("expected ROUTE_RESOLVED, got %s", rdma_event_str(event->event));
        rdma_ack_cm_event(event); goto out;
    }
    rdma_ack_cm_event(event);
